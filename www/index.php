...
if($rcelkem = mysqli_query($link, $celkem)){
    if(mysqli_num_rows($rcelkem) > 0){
            ...
            echo "<tr><th bgcolor='#03fce8'>Total</th><th bgcolor='#03fce8'><form method='get' action='/login.php'><button type='submit'>Patchmon Application</button></form></th></tr>";
		 while($row = mysqli_fetch_array($rcelkem)){
            echo "<tr><td>" . $row['celkem'] . "</td><td></td></tr>";
    }
}}    

$done='select done,count(*) as pocet from preview where LENGTH(done) > 0 group by done';	
echo "<br>";
